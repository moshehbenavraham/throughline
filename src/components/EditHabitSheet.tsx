...
export function EditHabitSheet({ habit, onClose, onSave, onDelete }: EditHabitSheetProps) {
  ...
  useEffect(() => {
    ...
    // Esc closes the dialog — keyboard parity with backdrop click.
    ...
    <div className="fixed inset-0 z-50">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 id={titleId} className="text-lg font-semibold text-foreground">Edit habit</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor={descId} className="text-sm font-medium text-foreground mb-1.5 block">Description (optional)</label>
            <input
              id={descId}
              type="text"
              value={description}
          ...
          <div>
            ...
            {showReminder && (
              <div className="mt-2">
                <label htmlFor="edit-reminder-time" className="sr-only">Reminder time</label>
                <input
          ...
          {onDelete && (
            <button
              ...
              aria-label={confirmDelete ? "Confirm delete habit" : "Delete habit"}
              className={`w-full flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium transition-all duration-200 active:scale-[0.98] outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-card ${
                confirmDelete
