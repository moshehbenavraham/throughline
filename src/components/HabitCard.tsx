  ...
  return (
    ...
    >
      ...
      <button
        ...
        type="button"
        {...listeners}
        className="absolute -left-6 top-1/2 -translate-y-1/2 w-5 h-8 flex flex-col items-center justify-center gap-[3px] cursor-grab active:cursor-grabbing opacity-30 hover:opacity-60 transition-opacity touch-none rounded outline-none focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background"
        aria-label={`Reorder ${habit.name}`}
      >
        <div className="w-1 h-1 rounded-full bg-foreground/40" aria-hidden="true" />
        <div className="w-1 h-1 rounded-full bg-foreground/40" aria-hidden="true" />
        <div className="w-1 h-1 rounded-full bg-foreground/40" aria-hidden="true" />
      </button>
      ...
      >
        ...
        <div className="relative flex items-center gap-3 p-4 pl-6">
          {/* Checkbox — 48px touch target. Keyboard focus is visible (focus-visible)
              while pointer clicks stay flat to preserve the calm aesthetic. */}
          <button
            type="button"
          ...
          >
            {completed && (
              <Check className="w-5 h-5 text-white animate-bounce-check" strokeWidth={2.5} aria-hidden="true" />
            )}
          </button>
          ...
          <button
            ...
            className="flex-1 min-w-0 text-left rounded-md outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
