...
export const Route = createFileRoute("/settings")({
  ...
  head: () =>
    // Settings is auth-walled — keep it out of SERPs.
    buildHead({
      title: "Continuum — Settings",
      description: "Customize your Continuum experience.",
      path: "/settings",
      noindex: true,
    }),
});
      ...
      <div className="max-w-lg mx-auto px-5 pt-12">
        ...
        <div className="mt-8 space-y-6">
          ...
          <section className="animate-fade-up-blur" style={{ animationDelay: "60ms" }}>
            <h2 id="appearance-heading" className="text-[11px] font-medium text-muted-foreground mb-2.5 uppercase tracking-wider">Appearance</h2>
            <div
              role="radiogroup"
              aria-labelledby="appearance-heading"
              className="rounded-2xl bg-card shadow-[0_1px_4px_0_rgba(0,0,0,0.06),0_0_0_1px_rgba(0,0,0,0.03)] p-1.5 flex gap-1"
            >
              {themeOptions.map(({ value, icon: Icon, label }) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-medium transition-all duration-300 active:scale-[0.97] outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card ${
                      isActive
                        ? "bg-primary text-primary-foreground shadow-sm"
                        : "text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    <Icon className={`w-4 h-4 transition-transform duration-300 ${isActive ? "rotate-0" : ""}`} aria-hidden="true" />
                    {label}
                  </button>
                );
              })}
            ...
            <section className="animate-fade-up-blur" style={{ animationDelay: "90ms" }}>
              ...
              <div className="rounded-2xl bg-card shadow-[0_1px_4px_0_rgba(0,0,0,0.06),0_0_0_1px_rgba(0,0,0,0.03)] overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-muted/30 transition-colors active:scale-[0.99] outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
                >
                  {notifPermission === "granted" ? (
                    <Bell className="w-4 h-4 text-primary flex-shrink-0" aria-hidden="true" />
                  ) : (
                    <BellOff className="w-4 h-4 text-muted-foreground flex-shrink-0" aria-hidden="true" />
                  )}
                  <div className="flex-1 min-w-0">
                  ...
                  <div
                    aria-hidden="true"
                    className={`w-10 h-6 rounded-full transition-colors ${notifPermission === "granted" ? "bg-primary" : "bg-muted"} flex items-center px-0.5`}
                  >
