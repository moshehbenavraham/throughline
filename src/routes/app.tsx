...
export const Route = createFileRoute("/app")({
  component: TodayPage,
  head: () =>
    // Auth-walled app shell — keep it out of search results.
    buildHead({
      title: "Continuum — Your daily ritual",
      description:
        "Build lasting habits with Continuum. Simple streak tracking, beautiful progress visualization.",
      path: "/app",
      noindex: true,
    }),
});
            ...
            {notScheduledToday.length > 0 && (
              <div className="mt-4">
                <button
                  ...
                  aria-expanded={showNotScheduled}
                  aria-controls="not-scheduled-list"
                  className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium mb-2 hover:text-foreground transition-colors rounded outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                >
                  <ChevronDown
                    aria-hidden="true"
                    className={`w-3.5 h-3.5 transition-transform duration-200 ${showNotScheduled ? "rotate-0" : "-rotate-90"}`}
                  />
                  Not scheduled today ({notScheduledToday.length})
                </button>
                {showNotScheduled && (
                  <div id="not-scheduled-list" className="space-y-2 opacity-60">
                    {notScheduledToday.map((habit) => (
                      <button
                        ...
                        className="flex items-center gap-3 rounded-xl bg-card/50 px-4 py-3 w-full text-left hover:bg-card/80 transition-colors outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                      >
                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: habit.color }} aria-hidden="true" />
                        <div className="flex-1 min-w-0">
                        ...
                        <Pencil className="w-3.5 h-3.5 text-muted-foreground/40" aria-hidden="true" />
                      </button>
                    ))}
                  </div>
                )}
